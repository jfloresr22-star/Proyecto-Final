...
public:
    ...
    void cargarDatosGuardados() {
        ...
        linea)) {
            ...
            getline(ss, dato, ',');
            nuevo->completion_year =
            stoi(dato);

            nuevo->siguiente =
            estudiante->dato.proyectos;
...
private:
    ...
    NodoAVL* buscarScore(NodoAVL* raiz, int score) {
        ...
        if (score < raiz->dato.skill_score)
            return buscarScore(raiz->izquierdo, score);

        return buscarScore(raiz->derecho, score);
    }
...
public:
    ...
    void buscarPorScore(int score) {
        ...
        NodoAVL* encontrado =
        buscarScore(raiz, score);

        if (encontrado == NULL) {

            cout << "Score no encontrado\n";
        ...
        cout << "ID: "
             << encontrado->dato.student_id
             << endl;

        cout << "Nombre: "
             << encontrado->dato.full_name
             << endl;

        cout << "Skill Score: "
             << encontrado->dato.skill_score
             << endl;
    }
    ...
    void benchmarkAVL(Student e) {
        ...
        inOrdenDescendente(raiz, c);

        auto finRecorrido = high_resolution_clock::now();

        archivo
        << "C++,traversal,AVL,10000,"
